...
The seventh downstream Backend Specification after BEB, immediately after BCART, MUST be the Category Backend Specification under scope `BCAT` at `specifications/backend/category/category-backend.md`. BCAT MUST use a Category-only decomposition and specialize exactly the Approved Category Domain without transferring Category Domain authority. BCAT MUST inherit every materially applicable BEB Requirement and explicitly trace that inheritance. It MAY consume materially applicable governed Contracts or evidence from existing Approved upstream Backend Specifications without transferring their authority. Category remains authoritative for Category-owned taxonomy, hierarchy, membership, classification, navigation, ordering, lifecycle, content, and other Category-owned behavior. Search and Discovery remains separate, and Checkout remains independently eligible and unresolved.
...
The eleventh downstream Backend Specification after BEB, immediately after BORD, MUST be the Shipping and Fulfilment Backend Specification under scope `BSHP` at `specifications/backend/shipping/shipping-backend.md`. BSHP MUST use a Shipping-and-Fulfilment-only decomposition and specialize exactly the Approved Shipping and Fulfilment Domain without transferring Shipping and Fulfilment Domain authority. BSHP MUST inherit every materially applicable BEB Requirement and explicitly trace that inheritance. It MAY consume materially applicable governed Contracts or evidence from existing Approved upstream Backend Specifications without transferring Identity, Customer, Product, Category, Inventory, Pricing, Cart, Checkout, Order, Payment, Return, CMS, Administration, Notifications, Reporting, Search and Discovery, or other Domain authority. BSHP is `1.0.0 Approved`; this lifecycle correction changes no BSHP authority or semantics.

The twelfth downstream Backend Specification after BEB, immediately after BSHP, MUST be the Payment Backend Specification under scope `BPAY` at `specifications/backend/payment/payment-backend.md`. BPAY MUST use a Payment-only decomposition and specialize exactly the Approved Payment Domain without transferring Payment Domain authority. BPAY MUST inherit every materially applicable BEB Requirement and explicitly trace that inheritance. It MAY consume materially applicable governed Contracts or evidence from existing Approved upstream Backend Specifications without transferring Identity, Customer, Product, Category, Search and Discovery, Inventory, Pricing, Cart, Checkout, Order, Shipping and Fulfilment, Return, CMS, Administration, Notifications, Reporting, or other Domain authority. BPAY exists at `specifications/backend/payment/payment-backend.md` as `1.0.0 Approved`, remains `authoritative: false`, and resolves no Open Product or Architecture Decision.

The thirteenth downstream Backend Specification after BEB, immediately after BPAY, MUST be the Return Backend Specification under scope `BRET` at `specifications/backend/return/return-backend.md`. BRET MUST use a Return-only decomposition and specialize exactly the Approved Return Domain without transferring Return Domain authority. BRET MUST inherit every materially applicable BEB Requirement and explicitly trace that inheritance. It MAY consume materially applicable governed Contracts or evidence from existing Approved upstream Backend Specifications without transferring Order, Inventory, Shipping and Fulfilment, Payment, Pricing, Identity, Customer, Administration, Notifications, Reporting, CMS, or other Domain authority. BRET exists at `specifications/backend/return/return-backend.md` as `1.0.0 Approved`, remains `authoritative: false`, and resolves no Open Product or Architecture Decision.
...
Approved BNTF closed Administration's Notifications backend invocation prerequisite. Approved BRPT closes Administration's previously missing Approved Reporting backend invocation Contract prerequisite without transferring Reporting authority. All materially required upstream backend invocation Contracts for Administration are now available through Approved Backend Specifications, and Administration is no longer dependency-blocked.
...
The initial PostgreSQL deployment MUST use one application database within the primary PostgreSQL service, with a dedicated PostgreSQL schema for each persistence-owning Module or Domain boundary. Every governed persistent object MUST remain attributable to one owning Module or Domain. Schema separation reinforces logical ownership and MUST NOT replace Module Contracts, Application Services, Ports and Adapters, Authorization, or other approved application boundaries.

Physical co-location in one database never authorizes direct cross-Module persistence access. Direct table or schema access across ownership boundaries remains prohibited except where a separate Accepted ADR and synchronized Architecture update explicitly authorizes the coupling under section 40.2. Flyway migration ownership MUST align with the owning Module or Domain, ordinary runtime capability MUST NOT perform DDL or schema administration, and migration or schema-changing capability MUST remain separately bounded under least privilege.

Because owned schemas remain within one database, an approved Database Transaction may technically span schemas when an approved Application Service or Use Case requires atomicity. This capability does not authorize arbitrary cross-Module writes or introduce distributed-transaction semantics. Dedicated schemas preserve identifiable ownership for possible future extraction but do not authorize database-per-Module, service extraction, microservices, or distributed transactions.
...
| 1.14.0 | 2026-09-21 | Approved | Synchronized Accepted ADR-0013 by establishing Payment-only BPAY immediately after BSHP, corrected BSHP to its existing 1.0.0 Approved state, recorded Return's Order, Inventory/restocking, and Shipping/reverse-logistics backend prerequisites as closed while preserving its unresolved Payment backend prerequisite, preserved CMS as independently eligible and unresolved, and left every post-BPAY roadmap position unresolved. |
