...
The twelfth downstream Backend Specification after BEB, immediately after BSHP, MUST be the Payment Backend Specification under scope `BPAY` at `specifications/backend/payment/payment-backend.md`. BPAY MUST use a Payment-only decomposition and specialize exactly the Approved Payment Domain without transferring Payment Domain authority. BPAY MUST inherit every materially applicable BEB Requirement and explicitly trace that inheritance. It MAY consume materially applicable governed Contracts or evidence from existing Approved upstream Backend Specifications without transferring Identity, Customer, Product, Category, Search and Discovery, Inventory, Pricing, Cart, Checkout, Order, Shipping and Fulfilment, Return, CMS, Administration, Notifications, Reporting, or other Domain authority. Accepted ADR-0013 authorizes BPAY to enter Draft lifecycle only; BPAY is not Approved, `specifications/backend/payment/payment-backend.md` need not exist yet, and BPAY MUST complete its own Draft-to-Approved lifecycle.

The canonical Backend Specification sequence is `BEB → BIDN → BCUS → BPRD → BINV → BPRC → BCART → BCAT → BSRCH → BCHK → BORD → BSHP → BPAY`.

CMS remains independently eligible, separate, unresolved, unranked, and unordered relative to unresolved capabilities. Return remains separate, unresolved, and unauthorized for Draft; its governed Order, Inventory/restocking, and Shipping/reverse-logistics backend prerequisites are closed, while its remaining identified backend prerequisite is an Approved Payment backend Contract. Selecting BPAY does not close that prerequisite; it closes only after a future BPAY Specification becomes Approved and exposes the applicable governed Contract. Administration, Notifications, and Reporting remain separate and unresolved. All Backend Specification identities, titles, paths, scope codes, decompositions, and ordering after BPAY remain unresolved until separately governed. ADR-0013 establishes no CMS, Return, Administration, Notifications, Reporting, or other post-BPAY backend position. Another governance decision is required unless canonical governance later uniquely determines the next position. All Backend Specifications remain subordinate to higher governing sources, Approved Business Requirements, Approved Domain Specifications, applicable Approved Frontend Specifications where Contracts intersect, standards under `.ai/backend/`, and materially applicable BEB Requirements.
